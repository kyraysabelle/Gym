class ClientWorkoutsController < ApplicationController
  ...
  def index
    ...
    @client_workouts = ClientWorkout.where('trainer=?', 'Kai').where('duration=?', '5')
    # @client_workouts = ClientWorkout.where('trainer=?', 'Kai')
  end
  ...
  private
    ...
    def client_workout_params
      params.require(:client_workout).permit(:client, :trainer, :duration, :dateofworkout, :paidamount)
    end
end
